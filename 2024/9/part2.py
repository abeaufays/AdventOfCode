    ...
    for idx, file in enumerate(file_map[::-1]):
        if file != EMPTY:
            if current_id is None:
                current_id, upper_bound = file, len(file_map) - idx - 1
        if current_id is not None and current_id != file:
            current_id = None
            yield (len(file_map) - idx, upper_bound)
...
test(next(get_last_filled_finder(test_value_as_disk_map)), (40, 41))
