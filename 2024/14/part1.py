...
class Robot:
    ...
    def get_position_quadrant(robot: Robot, width: int, height: int) -> int:
        is_topside = robot.position.x < width // 2
        is_botside = robot.position.x > width // 2
        is_leftside = robot.position.y < height // 2
        is_rightside = robot.position.y > height // 2
...
def get_map_context(filename: str) -> tuple[int, int, int]:
...
def get_data_from_file(filename: str) -> tuple[list[Robot], int, int, int]:
...
def compute_quadrants_scores(robots: list[Robot], width: int, height: int) -> list[int]:
    result = [0, 0, 0, 0]
    for robot in robots:
        result[robot.get_position_quadrant(width, height)] += 1
    return result


def answer(filename: str) -> int:
    robots, width, height, seconds = get_data_from_file(filename)
