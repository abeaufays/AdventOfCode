...
def answer(map_data: np.ndarray) -> int:
    ...
    for start_position in find_start_positions(map_data):
        visited = list()
        next_path_to_lookup = [[start_position]]
        while next_path_to_lookup:
            current_path = next_path_to_lookup.pop()
            visited.append(current_path)

            if map_data[current_path[-1]] == END:
                result += 1
                continue

            for adjacent_position in (
                add(current_path[-1], direction) for direction in DIRECTIONS
            ):
                if (
                    is_in_bound_of(adjacent_position, map_data)
                    and map_data[adjacent_position] == map_data[current_path[-1]] + 1
                    and adjacent_position not in visited
                ):
                    next_path_to_lookup.append(
                        current_path.copy() + [adjacent_position]
                    )
